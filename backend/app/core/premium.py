...
def is_premium_active(user: User) -> bool:
    ...
    Uwzględnia datę wygaśnięcia — jeśli `premium_expires_at` jest ustawione
    i minęło, traktujemy konto jako NIE-premium, nawet jeśli flaga
    `is_premium` wciąż jest ustawiona na True (na wypadek, gdyby proces
    odnawiania/wygaszania subskrypcji jeszcze nie zdążył zaktualizować tej
    flagi). `premium_expires_at = None` oznacza brak terminu wygaśnięcia
    (np. jednorazowy, bezterminowy zakup, jeśli kiedyś taki wprowadzimy).
    """
