...
class _AdminCampaignsScreenState extends State<AdminCampaignsScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      final start = DateTime(_dates!.start.year, _dates!.start.month, _dates!.start.day);
      final end = DateTime(_dates!.end.year, _dates!.end.month, _dates!.end.day + 1);
      await _client.post('/purchase-campaigns/admin', body: {
  ...
  Future<void> _toggle(Map<String, dynamic> campaign, bool value) async {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          actions: [
            ...
            FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Potwierdzam')),
          ],
        ),
      );
    ...
    try {
      await _client.patch('/purchase-campaigns/admin/${campaign['id']}?is_active=$value');
      await _load();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(friendlyError(e))));
    }
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(children: [
            ...
            CampaignIcon(icon: Icons.toll, color: AppTheme.accentColor, discountPercent: _percent),
          ]),
          const SizedBox(height: 20),
          ...
          DropdownButtonFormField<String>(
            ...
            items: const [
              DropdownMenuItem(value: 'premium', child: Text('Subskrypcja Premium')),
          ...
          DropdownButtonFormField<String>(
            value: _productId,
            decoration: const InputDecoration(labelText: 'Pakiet objęty ofertą'),
            items: (_kind == 'premium'
                ? const ['premium_weekly_v2', 'premium_monthly', 'premium_yearly']
          ...
          DropdownButtonFormField<int>(
            ...
            items: const [10, 20, 30, 40, 50].map((n) => DropdownMenuItem(value: n, child: Text('$n%'))).toList(),
          ...
          if (_audience == 'user') ...[
            const SizedBox(height: 12),
            TextField(controller: _email, keyboardType: TextInputType.emailAddress, decoration: const InputDecoration(labelText: 'E-mail użytkownika')),
          ],
          const SizedBox(height: 12),
          OutlinedButton.icon(
            onPressed: () async {
          ...
          const Text('Zapisane kampanie', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          if (_loading) const Center(child: CircularProgressIndicator()),
          if (_error != null) Text(_error!, style: TextStyle(color: AppTheme.errorColor)),
          ...
          for (final item in _campaigns)
            Card(child: Padding(
              ...
              child: Row(children: [
                CampaignIcon(
                  icon: item['kind'] == 'points' ? Icons.toll : Icons.workspace_premium,
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Row(children: [
                    ...
                    Switch.adaptive(
                      ...
                      onChanged: (value) => _toggle(item as Map<String, dynamic>, value),
                    ),
                    const Spacer(),
                    IconButton(
                      tooltip: 'Usuń kampanię',
                      onPressed: () => _delete(item as Map<String, dynamic>),
